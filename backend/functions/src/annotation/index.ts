...
export const getHTMLFromMarkdownStringArray = functions.https.onCall(
    ...
    // it contains one per annotation when we are reading an entire list
      ...
      // is this really any more performant than just running these sequentially?
      const htmlStrings = await Promise.all(
          markdownStrings.map((markdownString, idx) => {
            if (
          ...
          markdownString.length === 0
            ...
            try {
              return remark()
                  ...
                  }) // TODO: verify sanitize does not break anything
                  .use(htmlify, {sanitize: true})
                  .process(markdownString)
                  .toString();
            } catch (error) {
              throw new functions.https.HttpsError(
                  "internal",
                  `failed to process markdown[${idx}]\n${error}`
              );
            }
          })
      );
